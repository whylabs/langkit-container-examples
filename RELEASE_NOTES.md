...
We're introducing new, faster models for select topics: `code`, `medical`, and `financial`. Now, if you use these topics, either in the
WhyLabs policy UI or in a custom policy, the newer models will implement the generated metrics. You can still use arbitrary topics but
they won't be optimized and each one will add a constant amount of latency to the request.
...
And this is a ruleset based policy that enables them (typically generated by our platform as opposed to manually createing yaml).

```yaml
id: my-id
policy_version: 1
schema_version: 0.0.1
whylabs_dataset_id: model-178
